...
# To debug
#print("Temp Min "+str(min_temp)+" Max "+str(max_temp)+" Depth Min "+str(min_depth)+" Max "+str(max_depth)+" Pressure Min "+str(min_pressure)+" Max "+str(max_pressure));
#print(List_Depth)
...
with open(my_file) as csv_file:
    ...
    for row in csv_reader:
        if line_count == 0:
            ...
            #draw1.text((10, 0), "Dive number : "+row[0]+" ", font=fnt, fill=(25, 25, 75, 75), stroke_width=4, stroke_fill="black")
            #draw1 = ImageDraw.Draw(img)
            #draw1.text((10, 60), "Time : "+row[3]+" ", font=fnt, fill=(25, 25, 75, 75), stroke_width=4, stroke_fill="black")
            #draw2 = ImageDraw.Draw(img)
            #draw2.text((10, 120), "Depth : "+row[4]+" m ", font=fnt, fill=(25, 25, 75, 75), stroke_width=4, stroke_fill="black")
            #draw3 = ImageDraw.Draw(img)
            #draw3.text((10, 180), "Temperature : "+last_temp+" C ", font=fnt, fill=(25, 25, 75, 75), stroke_width=4, stroke_fill="black")
            #draw4 = ImageDraw.Draw(img)
            #draw4.text((10, 240), "Pressure : "+last_pressure+" Bar ", font=fnt, fill=(25, 25, 75, 75), stroke_width=4, stroke_fill="black")
    ...
    delta = {'reference': int(float(max_pressure)), 'increasing': {'color': "RebeccaPurple"}},
    gauge = {
        'axis': {'range': [None, 200], 'tickwidth': 1, 'tickcolor': my_color},
        'bar': {'color': my_color},
        ...
        'threshold': {
            ...
            'thickness': 0.75,
            'value': int(float(last_pressure))}}))
            fig.update_layout(paper_bgcolor = "rgba(0,0,0,0)", font = {'color': my_color, 'family': my_familly})
    ...
    y = List_Depth,line=dict(color="purple", width=10)))
            fig.add_hline(y=-int(float(row[4])), line_width=5, line_dash="dash", line_color="green")
            fig.add_vline(x=current_num_depth, line_width=5, line_dash="dash", line_color="green")
            fig.add_vrect(x0=0,x1=current_num_depth, line_width=0, fillcolor="red", opacity=0.3)
    ...
    value = line_count,
    domain = {'x': [0, 1], 'y': [0, 1]},
    title = {'text': 'Time '+time, 'font': {'size': my_font_size}}
            ))
                fig.update_layout(paper_bgcolor = "rgba(0,0,0,0)", font = {'color': my_color, 'family': my_familly})
                fig.write_image("Time.png",scale=my_scale)
                ...
                #draw1.text((10, 0), "Dive number : "+row[0]+" ", font=fnt, fill=(25, 25, 75, 75), stroke_width=4, stroke_fill="black")
                #draw1 = ImageDraw.Draw(img)
                #draw1.text((10, 60), "Time : "+time+" ", font=fnt, fill=(25, 25, 75, 75), stroke_width=4, stroke_fill="black")
                #draw2 = ImageDraw.Draw(img)
                #draw2.text((10, 120), "Depth : "+row[4]+" m ", font=fnt, fill=(25, 25, 75, 75), stroke_width=4, stroke_fill="black")
                #draw3 = ImageDraw.Draw(img)
                #draw3.text((10, 180), "Temperature : "+last_temp+" C ", font=fnt, fill=(25, 25, 75, 75), stroke_width=4, stroke_fill="black")
...
# Other test :
#os.system('/usr/bin/ffmpeg -r 1 -s 1280x720 -i tmpPNG/pic-%04d.png -crf 25 -pix_fmt yuva420p video.webm');
#os.system('/usr/bin/ffmpeg -r 1 -s 1280x720 -i tmpPNG/pic-%04d.png -c:v png -auto-alt-ref 0 -pix_fmt rgba video.mov');

# Good test :
print("4-Create video with picture");
os.system(my_ffmpeg+' -framerate 1 -start_number '+my_start_time+'  -s '+str(my_h)+'x'+str(my_w)+' -i '+my_tempFolder+'/pic-%04d.png -c:v png -r 30 '+my_videooutput);
...
os.system(my_ffmpeg+' -i '+my_videoinput+' -i '+my_videooutput+' -filter_complex "  [0:v]setpts=PTS-STARTPTS, scale='+str(my_h)+'x'+str(my_w)+'[top]; [1:v]setpts=PTS-STARTPTS, scale='+str(my_h)+'x'+str(my_w)+',  format=yuva420p,colorchannelmixer=aa=0.5[bottom];  [top][bottom]overlay=shortest=1"  -c:a aac -vcodec libx264 '+my_videooutputmerge);

print("6-(optionnal) Compress video");

os.system(my_ffmpeg+' -i '+my_videooutputmerge+' -vcodec libx265 -crf 28 '+my_videooutputmergecompress);
os.system(my_ffmpeg+' -i '+my_videooutput+' -vcodec libx265 -crf 28 '+my_videooutputcompress)
